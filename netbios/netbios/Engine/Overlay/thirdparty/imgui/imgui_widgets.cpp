...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    auto col = ImTricks::Animations::FastColorLerp(GetColorU32(ImGuiCol_WindowBg), GetColorU32(ImGuiCol_ChildBg), anim);

    if (g.LogEnabled)
        LogSetNextTextDecoration("[", "]");

    window->DrawList->AddRectFilled({ bb.Min.x, bb.Min.y }, bb.Max, GetColorU32(ImGuiCol_Scheme), SCALE(5), ImDrawFlags_RoundCornersAll);

    PushStyleColor(ImGuiCol_Text, col.Value);
    RenderText({ bb.GetCenter().x - label_size.x / 2, bb.GetCenter().y - label_size.y / 2 }, label);
    PopStyleColor();
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const float w = GetWindowWidth() - SCALE(30);
    const ImRect total_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, label_size.y + SCALE(13)));
    ...
    const ImRect bb(total_bb.Max - ImVec2(SCALE(24) + CalcTextSize(preview_value).x + (SCALE(150) - (SCALE(24) + CalcTextSize(preview_value).x)) * popup_open_anim, total_bb.GetHeight() + SCALE(6)), total_bb.Max - SCALE(ImVec2(0, 6)));
    ItemSize(total_bb, style.FramePadding.y);
    ...
    widget::ImRotateStart();
    RenderArrow(window->DrawList, { bb.Max.x - SCALE(14) - style.FramePadding.x * popup_open_anim, bb.GetCenter().y - SCALE(17) }, col, ImGuiDir_Right, SCALE(20.f));
    widget::ImRotateEnd(3.14f * popup_open_anim);

    RenderText({ total_bb.Min.x, total_bb.Max.y - label_size.y - SCALE(13) }, label);
    PushStyleColor(ImGuiCol_Text, col.Value);
    RenderText({ bb.Min.x + style.FramePadding.x * popup_open_anim, total_bb.Max.y - label_size.y - SCALE(13) }, preview_value);
    PopStyleColor();

    //window->DrawList->AddRectFilled({ total_bb.Min.x, total_bb.Max.y - SCALE(1) }, total_bb.Max, GetColorU32(ImGuiCol_Border));
...
bool ImGui::Combo(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int popup_max_height_in_items)
{
    ...
    for (int i = 0; i < items_count; i++)
    {
        ...
        if (Selectable(item_text, item_selected, ImGuiSelectableFlags_NoPadWithHalfSpacing, { 0, SCALE(GetIO().Fonts->Fonts[0]->FontSize) + SCALE(12) }))
        {
            value_changed = true;
            *current_item = i;
        }
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect total_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, label_size.y + SCALE(30)));
    const ImRect frame_bb(total_bb.Min + ImVec2(0, label_size.y + SCALE(12)), total_bb.Max - SCALE(ImVec2(0, 13)));
    ...
    static std::unordered_map< ImGuiID, float >* pValues = nullptr;
    if (pValues == nullptr) {
        pValues = new std::unordered_map< ImGuiID, float >();
    }
    std::unordered_map< ImGuiID, float >& values = *pValues;
    ...
    //绘制滑块
    window->DrawList->AddCircleFilled({ frame_bb.Min.x + value->second, frame_bb.GetCenter().y }, SCALE(8), GetColorU32(ImGuiCol_SliderGrab));
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    else if ((flags & ImGuiColorEditFlags_DisplayHex) != 0 && (flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        window->DC.CursorPos = { pos.x - SCALE(30), pos.y };
    ...
    {
        const float button_offset_x = window->Size.x - SCALE(50);
        window->DC.CursorPos = ImVec2(pos.x + button_offset_x, pos.y);

        const ImVec4 col_v4(col[0], col[1], col[2], alpha ? col[3] : 1.0f);
        if (ColorButton("##ColorButton", col_v4, flags, SCALE({ 20, 20 })))
        {
        ...
        PushStyleVar(ImGuiStyleVar_WindowPadding, SCALE({ 8, 8 }));
    ...
    //window->DrawList->AddRectFilled({ pos.x, pos.y + SCALE(25) }, { pos.x + window->Size.x - SCALE(30), pos.y + SCALE(26) }, GetColorU32(ImGuiCol_Border));
